...
InstallGlobalFunction(LowIndexSubgroups, function(G, maxIndex, numWorkers)
    local table,        # Coset table
          alphabet,     # Alphabet of coset table
    ...
    table[1] := ListWithIdenticalEntries(Size(alphabet), fail);
    ...
    for letter in [1..Size(alphabet)] do
        relsX[letter] := Filtered(relsC, w ->
                                 Subword(w,1,1) = alphabet[letter]);
    od;
    MakeImmutable(relsX);

    # Define a maximum number of cosets that may
    # be defined before a coincidence is forced
    ...
    workers := List([1..numWorkers], i->CreateThread(
                       Work,
                       workQueue,
                       resultsChan,
                       numJobs,
...
#####
# Work(workQueue, resultsChan, numJobs, finish,
#      alphabet, relsX, maxIndex, maxCosets)
# Checks the work queue for available tasks, calculates subgroups via
# ExecuteJob, and sends output to the results channel
#####
InstallGlobalFunction(Work, function(
        workQueue,
        resultsChan,
        numJobs,
        finish,
...
#####
# ExecuteJob(table, ...)
# Finds all the descendants of this coset table which correspond to
# subgroups of index at most maxIndex subject to all the relations in relsX.
#####
InstallGlobalFunction(ExecuteJob, function(
        table,
        alphabet,
        reps,
        gens,
    ...
    while Size(Filtered(table,t->t<>fail)) < maxCosets
      and not IsCompleteCosetTable(table) do
    ...
    if IsCompleteCosetTable(table)
       and Size(Filtered(table,t->t<>fail)) <= maxIndex then
...
InstallGlobalFunction(Scan, function(table, alphabet, label, a, rel, relsX)
    local f,        # Coset being inspected in forward scan
          ...
          prevPos,  # Alphabet pos of inverse of next character in backward scan
...
#####
# MakeAssignment(table, alphabet, reps, label, relsX, a, x, b)
...
#####
InstallGlobalFunction(MakeAssignment, function(
        table,
        alphabet,
        reps,
        label,
...
#####
# Coincidence(table, alphabet, label a, b, relsX)
# Combines two cosets a and b in the table, then scans them under relsX.
# Return false iff the label is violated
#####
InstallGlobalFunction(Coincidence, function(table, alphabet, label, a, b, relsX)
    local i,        # Row number in table
          ...
          rel;      # Loop variable for a relator
...
# Returns true iff this table could be the
# canonical representative of its conjugacy class
#####
...
# Returns true iff this coset table is complete (has no undefined values)
#####
